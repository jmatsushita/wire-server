...
data ConvMemberAddSizeChecked a = ConvMemberAddSizeChecked (NonEmpty a)
  deriving (Functor, Foldable, Traversable)

checkedConvSize :: [a] -> Galley (ConvSizeChecked a)
...
class CanCheckSize f where
  checkedMemberAddSize :: f a -> Galley (ConvMemberAddSizeChecked a)

instance CanCheckSize NonEmpty where
  checkedMemberAddSize new = do
    limit <- view (options . optSettings . setMaxConvSize)
    if within new 1 (fromIntegral limit)
      then pure $ ConvMemberAddSizeChecked new
      else throwErr (errorMsg (1 :: Integer) limit "")

instance CanCheckSize [] where
  checkedMemberAddSize ls = case nonEmpty ls of
    Nothing -> throwErr "List of members (local or remote) to be added must be of at least size 1"
    Just new -> checkedMemberAddSize new
