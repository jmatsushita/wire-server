...
joinConversation zusr zcon cnv access = do
  ...
  let newUsers = map unTagged (filter (notIsConvMember conv) [luid])
  ...
  now <- liftIO getCurrentTime
  addToConversation luid zcon now (map (,roleNameWireMember) newUsers) conv
...
addMembers zusr zcon convId invite = do
  ...
  let (invitedRemotes, invitedLocals) = partitionRemoteOrLocalIds' (lDomain luid) invitedUsers
      newLocals = filter (notIsConvMember conv) invitedLocals
      newRemotes = filter (notIsConvMember conv) invitedRemotes
      newMembers = map (unTagged . qualifyAs luid) newLocals <> map unTagged newRemotes
...
addToConversation ::
  ...
  Data.Conversation ->
  Galley (UpdateResult Event)
addToConversation luid con now newMembers conv = case nonEmpty newMembers of
  Nothing -> pure Unchanged
  Just new -> do
    ensureGroupConvThrowing conv
    mems <- checkedMemberAddSize new
    ...
    let targets = ntAddMembers lmems rmems (convTargets conv)
        action = ConversationActionAddMembers new
...
ensureMemberLimit :: [LocalMember] -> [Qualified UserId] -> Galley ()
